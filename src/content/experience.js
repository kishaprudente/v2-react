const experience = [
	{
		title: 'Web & Mobile Developer',
		subtitle: 'BoomNation | Remote',
		description:
			'The largest database of skilled workers, tradesman, and truckers in the world. A community focused on finding jobs, upskilling, and reskilling',
		bullets: [
			'- Technologies used: Flutter, GraphQL, Firebase, Angular, Typescript, Azure',
			'- Effectively coded software changes and alterations based on specific design specifications.',
			'- Brought forth a passion and dedication to software development.',
			'- Collaborate with the team using SCRUM-based approach',
		],
		date: '(2020-Present)',
		icon: 'ti-pencil-alt',
	},
	{
		title: 'Coding Tutor',
		subtitle: 'Trilogy Education | Remote',
		description:
			'One-on-one tutor offering remote, supplemental educational assistance to students in Trilogy educational bootcamps nationwide.',
		bullets: [
			'- Topics covered: JavaScript, NodeJS, ReactJS, ExpressJS, Bootstrap, HTML5, CSS3, Git, GitHub, SQL, NoSQL, Object Oriented Programing.',
			'- Provide emotional support to students strugling to understand coding fundamentals.',
			'- Guide students to understand concepts of coding.',
		],
		date: '(2020-Present)',
		icon: 'ti-heart',
	},
];

export default experience;
